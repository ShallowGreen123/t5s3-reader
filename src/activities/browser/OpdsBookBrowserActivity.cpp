...
bool OpdsBookBrowserActivity::onTouchTap(int16_t x, int16_t y) {
  ...
  const int row = (y - 60) / rowHeight;
  const int touchedIndex = selectorIndex / PAGE_ITEMS * PAGE_ITEMS + row;
  if (row < 0 || row >= PAGE_ITEMS || touchedIndex < 0 || touchedIndex >= static_cast<int>(entries.size())) {
    return false;
  }
