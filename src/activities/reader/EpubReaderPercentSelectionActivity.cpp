...
bool EpubReaderPercentSelectionActivity::onTouchTap(int16_t x, int16_t y) {
  ...
  if (y >= barY - 16 && y <= barY + barHeight + 16 && x >= barX && x <= barX + barWidth) {
    percent = std::clamp(((x - barX) * 100) / std::max(1, barWidth), 0, 100);
    setResult(PercentResult{percent});
    finish();
    return true;
  }
