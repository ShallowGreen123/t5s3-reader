...
bool ButtonRemapActivity::onTouchTap(int16_t, int16_t y) {
  ...
  if (rowHeight > 0 && y >= topOffset && y < topOffset + contentHeight) {
    const int pageItems = std::max(1, contentHeight / rowHeight);
    const int row = (y - topOffset) / rowHeight;
    if (row >= 0 && row < pageItems && row < kRoleCount) {
  ...
  const int resetHintY = topOffset + 4 * metrics.listRowHeight + 4 * metrics.verticalSpacing;
  const int cancelHintY = topOffset + 4 * metrics.listRowHeight + 5 * metrics.verticalSpacing + 20;
  if (y >= resetHintY && y < resetHintY + 24) {
    SETTINGS.frontButtonBack = CrossPointSettings::FRONT_HW_BACK;
    SETTINGS.frontButtonConfirm = CrossPointSettings::FRONT_HW_CONFIRM;
    SETTINGS.frontButtonLeft = CrossPointSettings::FRONT_HW_LEFT;
    SETTINGS.frontButtonRight = CrossPointSettings::FRONT_HW_RIGHT;
  ...
  if (y >= cancelHintY && y < cancelHintY + 24) {
    finish();
    return true;
  }
