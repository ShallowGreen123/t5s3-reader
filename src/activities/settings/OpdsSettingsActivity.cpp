...
bool OpdsSettingsActivity::onTouchTap(int16_t, int16_t y) {
  ...
  const int menuItems = getMenuItemCount();
  if (menuItems <= 0 || rowHeight <= 0 || y < contentTop || y >= contentTop + contentHeight) {
    return false;
  }

  const int pageItems = std::max(1, contentHeight / rowHeight);
  const int row = (y - contentTop) / rowHeight;
  const int pageStartIndex = (static_cast<int>(selectedIndex) / pageItems) * pageItems;
  const int touchedIndex = pageStartIndex + row;
  if (row < 0 || row >= pageItems || touchedIndex < 0 || touchedIndex >= menuItems) {
    return false;
  }

  selectedIndex = static_cast<size_t>(touchedIndex);
  handleSelection();
  return true;
}
