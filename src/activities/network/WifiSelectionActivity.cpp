...
bool WifiSelectionActivity::onTouchTap(int16_t x, int16_t y) {
  ...
  if (state == WifiSelectionState::NETWORK_LIST) {
    ...
    const int pageStartIndex = (static_cast<int>(selectedNetworkIndex) / pageItems) * pageItems;
    const int touchedIndex = pageStartIndex + row;
    if (row < 0 || row >= pageItems || touchedIndex < 0 || touchedIndex >= static_cast<int>(networks.size())) {
      return false;
    }

    selectedNetworkIndex = static_cast<size_t>(touchedIndex);
  ...
  if (state == WifiSelectionState::SAVE_PROMPT) {
    ...
    const int top = (pageHeight - height * 3) / 2;
    ...
    if (y >= buttonY - 8 && y <= buttonY + height + 8) {
      if (x >= startX && x < startX + buttonWidth) {
        savePromptSelection = 0;
      } else if (x >= startX + buttonWidth + buttonSpacing &&
