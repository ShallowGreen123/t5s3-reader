...
bool KeyboardEntryActivity::onTouchTap(int16_t x, int16_t y) {
  ...
  for (int i = 0; i < BOTTOM_KEY_COUNT; i++) {
    ...
    if (x >= keyX && x < keyX + bottomKeyWidth && y >= bottomRowY && y < bottomRowY + bottomKeyHeight) {
  ...
  const int inputFieldBottom = inputStartY + inputHeight + lineHeight + metrics.verticalSpacing;
  if (y >= inputStartY - 8 && y <= inputFieldBottom) {
    cursorMode = true;
    togglePos = false;
    cursorPos = text.length();
    hintVisible = true;
